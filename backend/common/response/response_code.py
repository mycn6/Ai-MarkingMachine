...
class CustomCodeBase(Enum):
    ...
    # @property 是 Python 内置装饰器，用于创建属性访问器，允许你将方法定义为类属性进行访问。
    # 封装性：隐藏内部实现细节，只暴露必要的接口，使得调用者只关注接口，而不必关注实现。
    # 公共逻辑放在基类中
    # 子类只需定义具体状态码
    @property
    def code(self):
...
# @dataclass 是 Python 3.7+ 在 dataclasses 模块中引入的装饰器，用于自动为类添加特殊方法（如 __init__, __repr__ 等），从而简化数据类的创建
